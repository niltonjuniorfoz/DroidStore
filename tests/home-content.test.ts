...
test("configuração salva substitui os textos sem perder campos ausentes", () => {
  ...
  assert.equal(
    readHomeFooterBanner(value).imageUrl,
    DEFAULT_HOME_FOOTER_BANNER.imageUrl,
  );
  assert.equal(
    readHomePromoBanners(value)[0].title,
    "iPhones selecionados",
  );
  assert.equal(
    readHomePromoBanners(value)[0].buttonLabel,
    DEFAULT_HOME_PROMO_BANNERS[0].buttonLabel,
  );
  assert.equal(
    readHomeProductSections(value)[0].query,
    "xiaomi, poco",
  );
  assert.equal(
    readHomeProductSections(value)[1].title,
    DEFAULT_HOME_PRODUCT_SECTIONS[1].title,
  );
});
...
test("upload de imagens da vitrine permanece disponível sem Vercel Blob", () => {
  ...
  const uploadRoute = readFileSync(
    join(root, "app", "api", "admin", "upload", "route.ts"),
    "utf8",
  );
  const mediaRoute = readFileSync(
    join(root, "app", "api", "media", "admin-upload", "[id]", "route.ts"),
    "utf8",
  );
  const uploadClient = readFileSync(
    join(root, "src", "lib", "uploadClient.ts"),
    "utf8",
  );
...
test("menu legado de marcas migra para departamentos editáveis", () => {
  ...
  assert.deepEqual(
    resolveStorefrontNavigation(legacy),
    DEFAULT_STOREFRONT_NAVIGATION.map(({ label, href }) => ({
      label,
      href,
    })),
  );
  ...
  assert.deepEqual(
    DEFAULT_STOREFRONT_NAVIGATION.map((item) => item.label),
    [
      "Smartphones",
      "Informática",
      "Eletrônicos",
...
test("mega menu público usa a navegação salva no painel", () => {
  ...
  const megaMenu = readFileSync(
    join(root, "src", "components", "MegaMenu.tsx"),
    "utf8",
  );
  const admin = readFileSync(
    join(root, "app", "admin", "conteudo", "page.tsx"),
    "utf8",
  );
